 ...
 *              For example ./dsh -c 10.50.241.18:5678 where 5678 is the new
 * port number and the server address is 10.50.241.18
 *
 ...
 *                          is_eof = (recv_buff[recv_bytes-1] == RDSH_EOF_CHAR)
 * ? 1 : 0; if is_eof is true, this is the last part of the transmission from
 * the server and you can break out of the recv() loop.
 *
 *   returns:
 ...
 * ERR_RDSH_COMMUNICATION); return client_cleanup(cli_socket, request_buff,
 * resp_buff, OK);
 *
 *   The above will return ERR_RDSH_COMMUNICATION and OK respectively to the
 * main() function after cleaning things up.  See the documentation for
 * client_cleanup()
 *
 */
