...
#define RDSH_DEF_CLI_CONNECT                                                   \
  "127.0.0.1" // Default server is running on
              // localhost 127.0.0.1

// constants for buffer sizes
#define RDSH_COMM_BUFF_SZ (1024 * 64) // 64K
#define STOP_SERVER_SC                                                         \
  200 // returned from pipeline excution
      // if the command is to stop the
      // server.  See documentation for
      // exec_client_requests() for more info

// end of message delimiter.  This is super important.  TCP is a stream,
// therefore the protocol designer is responsible for managing where messages
// begin and end there are many common techniques for this, but one of the
// simplest ways is to use an end of stream marker.  Since rsh is a "shell"
// program we will be using ascii code 0x04, which is commonly used as the
// end-of-file (EOF) character in linux based systems.
...
#define CMD_ERR_RDSH_SEND                                                      \
  "rdsh-error: partial send.  Sent %d, expected to send %d\n"
