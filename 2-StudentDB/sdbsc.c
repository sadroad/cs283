 ...
 */
int get_student(int fd, int id, student_t *s) { return NOT_IMPLEMENTED_YET; }
...
int print_db(int fd) {
  ...
  while ((bytes_read = read(fd, &buffer, STUDENT_RECORD_SIZE)) > 0) {
    if (memcmp(&buffer, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
      if (!header_printed) {
        printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST NAME",
               "GPA");
        header_printed = true;
      }
 ...
 *
 */
void print_student(student_t *s) { printf(M_NOT_IMPL); }
